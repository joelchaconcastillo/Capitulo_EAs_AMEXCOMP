...
%
Inicialmente, recibe a la población padre (vectores objetivo) a la población de hijos (vectores de prueba) y a los vectores elite.
%
...
donde $D$ es la dimensión del problema, y $x_d^{(L)}, x_d^{(U)}$ son los límites menores y mayores de cada dimensión ($d$).
%
...
%
En su lugar, se realizó un análisis previo para determinar un valor adecuado, estableciéndose así el valor $D_I = 0.3$ ya que reporta resultados de calidad.
%
Por lo tanto todos los resultados que se muestran en este capítulo consideraron dicho valor para $D_I$.
...
%
Por otra parte, los valores elevados de $CR$ en general, proporcionan soluciones de mayor calidad con una menor probabilidad, ya que en algunas veces se realizan desplazamientos largos y exitosos que en consecuencia puedan mejorar la aptitud de forma significativa.
...
parametrización que \DEEDM{} pero que no incluye la población elite ni el reemplazamiento basado en diversidad.
%
...
Específicamente, $SE_i$ es la sumatoria de errores del $i$-ésimo algoritmo ($SE_i = \sum_{j=1}^{30} error\_\hat{f}_j$), además $error\_\hat{f}_j$ es el error promedio de la $j$-ésima función.
%
